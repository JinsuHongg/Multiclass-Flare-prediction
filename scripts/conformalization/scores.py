...
class cp:
    ...
    def label_q(self, cal_dict: Dict[str, np.ndarray]) -> float:
        ...
        """
        scores: np.ndarray = 1 - self.find_predprob_truelabel(softmax = cal_dict['softmax'], label = cal_dict['label'])
    ...
    def aps_q(self, cal_dict: Dict[str, np.ndarray]) -> float:
        ...
        arr_scores = self.find_cumsum_descending(softmax = cal_dict['softmax'])[np.arange(n), cal_dict['label']]
    ...
    def mcp_q(self, cal_dict: Dict[str, np.ndarray], type: str = 'label') -> float:
        ...
        if type == 'label':
            ...
            for label in np.unique(cal_dict['label']):
                ...
                scores: np.ndarray = 1 - self.find_predprob_truelabel(softmax = softmax_subset, label = label_subset)
                # confidence_corrected: float = np.ceil((n + 1) * self.confidence) / n
            ...
            for label in np.unique(cal_dict['label']):
                ...
                arr_scores = self.find_cumsum_descending(softmax = softmax_subset)[np.arange(n), label_subset]
    ...
    def mcp_region(self, val_dict: Dict[str, np.ndarray], type: str = 'label') -> np.ndarray:
        """
        Computes the prediction region for the validation set.
        ...
        if self.q_hat_dict[type] is None:
            raise ValueError("q_hat_dict is not computed. Call mcp_q() before mcp_region().")
        elif len(self.q_hat_dict[type]) != n_classes:
            raise ValueError("q_hat_dict does not have four components. Please check your input npz file.")
        
        softmax_arr = val_dict['softmax'].copy()  # Avoid modifying the original data

        if type == 'label':
            for fl_class, q_hat in enumerate(self.q_hat_dict['label']):
                softmax_arr[:, fl_class] = np.where(softmax_arr[:, fl_class] >= (1 - q_hat), softmax_arr[:, fl_class], 0)

        elif type == 'aps':
            val_arr_sort = self.find_cumsum_descending(softmax=val_dict['softmax'])  # Ensure correct method call
            for fl_class, q_hat in enumerate(self.q_hat_dict['aps']):  # Corrected dictionary key
                softmax_arr[:, fl_class] = softmax_arr[:, fl_class] * (val_arr_sort[:, fl_class] <= q_hat)
    ...
    def compute_q_hat(scores: np.ndarray, confidence) -> float:
        confidence_corrected: float = np.ceil((scores.shape[0] + 1) * confidence) / scores.shape[0]
        return float(np.quantile(scores, confidence_corrected, method='higher'))
    
def coverage_and_length(pred_region: np.ndarray, label: np.ndarray) -> Tuple[float, float]:
    ...
    avg_cov = np.mean(pred_region[np.arange(len(label)), label] != 0)
