...
# define arguments
args = get_args(
    config_dir="./Multiclass-Flare-prediction/scripts/configs/cp_config_ensemble.yaml"
)
...
print(
    f"learning rate: {args.lr:.1e}, max learning rate: {args.max_lr}, division factor: {args.div_factor:.1e}, decay value: {args.wt_decay}"
)
...
for _ in range(args.num_models):
    sampler = BootstrapSampler(data_train)  # Create a bootstrap sample
    dataloader = DataLoader(data_train, batch_size=args.batch_size, sampler=sampler)
...
for i_model, train_dataloader in enumerate(train_dataloaders):
        ...
        for t in range(args.epochs):
            ...
            if best_hsstss < check_hsstss:
                ...
                PATH = (
                    ...
                    + f"{args.model}_{year}{month:02d}_train{''.join(args.train_set)}_test4_{args.file_tag}_{i_model}.pth"
                )
                # save model
                torch.save(
                    {
                ...
                pred_path = (
                    ...
                    + f"{args.model}_{year}{month:02d}_train{''.join(args.train_set)}_test4_{args.file_tag}_{i_model}.npy"
                )
    ...
    total_save_path = (
        ...
        + f"{args.model}_{year}{month:02d}_validation_{args.file_tag}_{i_model}_results.csv"
    )
